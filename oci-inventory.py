...
def check_linux_agents(ip):
    """
    Linux agent check via SSH with 4-state output:
      - Yes           : installed & running
      - No            : installed but stopped
      - Not Installed : agent not present
      - SSH Fail      : cannot connect via SSH
    """

    # Default values
    result = {"Qualys": "Not Installed", "MDE": "Not Installed"}

    # -------------------------------------------------
    # STEP 1: Basic SSH reachability test
    # -------------------------------------------------
    rc, out, err = run_ssh_command(ip, "echo SSH_OK")
    if rc != 0:
        # SSH itself failed → explicitly report SSH Fail
        result["Qualys"] = "SSH Fail"
        result["MDE"] = "SSH Fail"
        return result

    # -------------------------------------------------
    # STEP 2: MDE (Microsoft Defender for Endpoint)
    # -------------------------------------------------
    # Check if service exists
    rc, out, err = run_ssh_command(
        ip,
        "/bin/systemctl list-unit-files mdatp.service 2>/dev/null | grep -q mdatp && echo PRESENT || echo ABSENT"
    )
    ...
    if out.strip() == "PRESENT":
        ...
        result["MDE"] = "Not Installed"

    # -------------------------------------------------
    # STEP 3: Qualys
    # -------------------------------------------------
    # Check install path (strong indicator)
    rc, out, err = run_ssh_command(
        ip,
        "test -x /usr/local/qualys/cloud-agent/bin/qualys-cloud-agent && echo PRESENT || echo ABSENT"
    )

    if out.strip() == "PRESENT":
        # Installed → check running process
        rc, out, err = run_ssh_command(
            ip,
            "pgrep -fa 'qualys-cloud-agent|qualys-cep' 2>/dev/null | head -n 1"
        )
...
def collect_object_storage(object_client, monitoring_client, compartment_id, region):
    """
    Collect Object Storage bucket inventory (safe for all OCI SDK versions)
    """
    rows = []

    try:
        namespace = object_client.get_namespace().data
        buckets = object_client.list_buckets(
            namespace_name=namespace,
            compartment_id=compartment_id
        ).data
    except Exception as e:
        print(f"Error listing buckets in {region}: {e}")
        return rows

    for b in buckets:
        try:
            bucket = object_client.get_bucket(
                namespace_name=namespace,
                bucket_name=b.name
            ).data
        except Exception as e:
            print(f"Error fetching bucket {b.name}: {e}")
            continue
        ...
        # -------- Auto Tiering (SDK-safe) --------
        auto_tiering = getattr(bucket, "is_auto_tiering_enabled", None)
        auto_tiering_state = "Enabled" if auto_tiering else "Disabled"

        # -------- Lifecycle Policy --------
        lifecycle_state = "Present" if getattr(bucket, "lifecycle_policy", None) else "Not Present"

        # -------- Bucket Size Metric --------
        stored_gb = ""
        try:
            query = f"""
            StoredBytes[1h]{{resourceId="{bucket.id}"}}.mean()
            """
            ...
            if resp.data and resp.data[0].aggregated_datapoints:
                stored_gb = round(
                    resp.data[0].aggregated_datapoints[-1].value / (1024 ** 3), 2
                )
        except Exception:
            pass

        created_time = bucket.time_created
        if isinstance(created_time, datetime) and created_time.tzinfo:
        ...
        rows.append({
            ...
            "Bucket OCID": bucket.id,
            "Storage Tier": bucket.storage_tier,
            "Versioning": versioning_state,
            "Auto Tiering": auto_tiering_state,
            "Lifecycle Policy": lifecycle_state,
            "Stored Size (GB)": stored_gb,
...
#Alarms
def collect_alarms(monitoring_client, compartment_id, region):
    """
    Collects OCI Monitoring Alarms using list_alarms().
    Uses AlarmSummary attributes ONLY (safe for inventory).
    """
    rows = []
    ...
    try:
        ...
        print(f"Error listing alarms in {region}: {e}")
        return rows

    for a in alarms:
        rows.append({
            "Region": region,
            "Alarm Name": a.display_name,
            "Alarm OCID": a.id,
            "Metric Namespace": a.namespace,
            "Query": a.query,
            "Severity": a.severity,
            "Enabled": "Yes" if a.is_enabled else "No",
            "Destinations": ", ".join(a.destinations or []),
            "Resource Group": a.resource_group or ""
        })
...
def main():
   ...
   # win_password = getpass.getpass("Enter Windows ociadmin password: ")
    ...
    all_instances, all_vcns_subnets, all_backups, all_dbs, all_osmh, all_osmh_summary, all_object_storage, all_alarms = [], [], [], [], [], [], [], []
    ...
    for region in regions:
        ...
        obj_rows = collect_object_storage(
            object_client,
            monitoring_client,
            root_compartment_id,
            region
        ...
        print(f"  Found {len(obj_rows)} Object Storage buckets")
        all_object_storage.extend(obj_rows)
        
        alarm_rows = collect_alarms(
            monitoring_client,
            comp_id,
            region
        )
        print(f"  Found {len(alarm_rows)} alarms")
    ...
    with pd.ExcelWriter(out_file, engine="openpyxl") as writer:
        ...
        pd.DataFrame(all_object_storage).to_excel(writer, sheet_name="ObjectStorage", index=False)
        pd.DataFrame(all_alarms).to_excel(writer, sheet_name="Alarms", index=False)

    print(
    f"Exported {len(all_instances)} instances, "
    f"{len(all_vcns_subnets)} VCN/Subnets, "
    f"{len(all_backups)} backups, "
    f"{len(all_dbs)} databases, "
    f"{len(all_object_storage)} buckets, "
    f"{len(all_alarms)} alarms "
